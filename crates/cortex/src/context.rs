...
1. TRUTH OVER HALLUCINATION: Ground answers in (a) the provided memories, (b) the live conversation history above this message, and (c) general world knowledge. If a *fact about the user* is genuinely absent from memory AND not present in the conversation, state: "I don't have that in my memory yet." Do NOT say this when the user is asking about things discussed earlier in the current conversation — answer from the message thread itself.
   ...
   - MEMORY WRITES: Claim you saved, added, noted, remembered, or "will remember" something ONLY when there is direct evidence of the write in your context THIS turn: either it appears in a "Saved this turn" block below, or you called the `memory.store` tool this turn and it returned success. If neither is true, NEVER assert a save — instead offer to make one ("Want me to remember that?"). A confirmation of a save that did not happen is a fabrication and breaks the core promise of a memory product.
...
mod tests {
    ...
    #[test]
    fn system_prompt_grounds_memory_write_claims_on_evidence() {
        // The reasoner may confirm a save only with this-turn evidence (a
        // "Saved this turn" block or a successful memory.store call); otherwise
        // it must offer to remember rather than fabricate a save.
        let assembler = ContextAssembler::with_defaults();
        let messages = assembler.assemble(
            "the staging server is at 10.4.2.19, I keep forgetting it",
            &[],
            &[],
        );
        let system = &messages[0].content;
        assert!(
            system.contains("MEMORY WRITES"),
            "memory-write grounding rule missing from system prompt"
        );
        assert!(
            system.contains("Saved this turn"),
            "write rule must anchor save-claims on the 'Saved this turn' grounding block"
        );
        assert!(
            system.contains("Want me to remember that?"),
            "write rule must direct the reasoner to offer a save when there is no evidence"
        );
    }
