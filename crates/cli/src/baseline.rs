...
//! process environment, and the pure native-capability descriptor list. There
//! is no LLM and no network: the snapshot is exactly what the running config
//! declares, and a diff is a literal set comparison of two such snapshots.
...
use crate::capabilities::{native_descriptors, WiredNatives};
...
pub async fn cmd_baseline(config: &BrainConfig, action: BaselineAction) -> Result<()> {
    match action {
        BaselineAction::Capture { label } => cmd_capture(config, label),
        BaselineAction::Diff { from, to } => cmd_diff(config, from, to),
        BaselineAction::List => cmd_list(config),
    }
}

// ─── capture ──────────────────────────────────────────────────────────────────

fn cmd_capture(config: &BrainConfig, label: Option<String>) -> Result<()> {
...
/// pure native-capability descriptors. No I/O beyond reading `config` — so the
/// result is deterministic and stable across runs of the same config.
fn capture_facts(config: &BrainConfig) -> BTreeMap<String, String> {
    ...
    for d in native_descriptors(WiredNatives::from_config(config)) {
        put(
            &format!("capability.{}.{}", d.verb.namespace, d.verb.action),
            d.usage
                .tier
                .clone()
...
fn cmd_diff(config: &BrainConfig, from: Option<u32>, to: Option<u32>) -> Result<()> {
    let dir = baseline_dir(config);
    ...
    let (target_facts, target_label) = match to {
        ...
        None => (capture_facts(config), "current live state".to_string()),
    };
    ...
    println!("Drift: {from_label}  →  {target_label}");
    if let Some(l) = &base.label {
        println!("  from label: {l}");
    }
...
fn cmd_list(config: &BrainConfig) -> Result<()> {
    let dir = baseline_dir(config);
    ...
    println!("Baselines in {}:", dir.display());
    for b in &baselines {
        let label = b
            .label
            .as_deref()
...
mod tests {
    ...
    fn capture_is_deterministic_for_one_config() {
        let cfg = test_config();
        assert_eq!(capture_facts(&cfg), capture_facts(&cfg));
    }

    #[test]
    fn capture_records_core_identity_facts() {
        let facts = capture_facts(&test_config());
        assert_eq!(
        ...
        // Always-on natives are present as capability facts with their tier.
    ...
    #[test]
    fn disabling_an_action_drops_its_capability_fact() {
        let mut cfg = test_config();
        cfg.actions.web_search.enabled = true;
        let with_web = capture_facts(&cfg);
        cfg.actions.web_search.enabled = false;
        let without_web = capture_facts(&cfg);

        assert!(with_web.contains_key("capability.net.http"));
        assert!(!without_web.contains_key("capability.net.http"));

        // And the diff surfaces it as a removed fact (plus the toggle change).
        ...
        assert!(drift
            .changed
            .iter()
            .any(|(k, old, new)| k == "actions.web_search" && old == "true" && new == "false"));
    }
    ...
    fn next_version_increments_and_round_trips() {
        ...
        let b = Baseline {
            ...
            facts: capture_facts(&test_config()),
        };
        std::fs::write(
            baseline_path(p, 1),
