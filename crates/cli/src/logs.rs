//! `brain logs analyze` — log pattern detection (Issue 137).
//!
...
//!    invents anything, it only counts what is there.
//!
//! 2. **Optional LLM narration** (skipped with `--no-llm` or when no provider is
//!    reachable). The deterministic digest — not the raw log — is handed to the
//!    LLM, which renders a short plain-language summary. Because it only ever
//!    sees the digest, it cannot fabricate log lines or causes; it narrates the
//!    counts it is given and degrades gracefully to the digest alone.
//!
//! Sources: `--source brain` (default) reads the daemon's own rotated logs in
//! `~/.brain/logs/`; `--source system` reads OS logs (`log show` on macOS,
...
pub async fn cmd_logs(config: &BrainConfig, action: LogsAction) -> Result<()> {
    match action {
        LogsAction::Analyze {
            ...
            lines,
            no_llm,
        } => cmd_logs_analyze(config, source, &since, lines, no_llm).await,
    }
}

async fn cmd_logs_analyze(
    config: &BrainConfig,
    source: LogSource,
    since: &str,
    lines: usize,
    ...
    match narrate(config, &digest).await {
        Ok(summary) => {
            println!("\nSummary");
            println!("{summary}");
        }
        Err(e) => {
            // The deterministic digest above is the real deliverable; narration
            // is a bonus. Say plainly why it's missing rather than failing.
...
/// timeout-bounded child process instead — the only way to make `--source
/// system` work there.
...
fn system_log_command(_since: &str) -> Result<(String, Vec<String>, String)> {
    anyhow::bail!("--source system is only supported on macOS and Linux")
}
...
impl Digest {
    ...
    /// The compact text handed to the LLM for narration — the same digest a
    /// human sees, so the model can only narrate counts that are actually here.
...
/// Narrate the digest in plain language. The prompt is strictly grounded: the
/// model is told to summarise only what the digest contains and never to invent
/// log lines or causes. Returns an error (not a panic) when no provider is
/// reachable, so the caller can degrade to the digest alone.
async fn narrate(config: &BrainConfig, digest: &Digest) -> Result<String> {
