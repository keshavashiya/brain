...
//!
//! The security analog of `brain baseline`: a deterministic, offline pass over
//! the *loaded config* that emits severity-ranked findings about the running
//! posture — network exposure, authentication, egress, secret handling, the
//! execution sandbox surface, and at-rest encryption. Truthful by construction:
//! it reads only `config`, never the network or an LLM, so every finding is a
//! literal consequence of what the config declares.
//!
//! The same pure [`audit`] function backs both surfaces — the CLI prints
//! [`render`]; [`ConfigSecurityAuditor`] wraps it to satisfy the
//! [`cortex::actions::SecurityAuditBackend`] trait the chat tool-loop dispatches
//! `security.audit` through.
...
pub fn audit(config: &BrainConfig) -> Vec<Finding> {
    ...
    if http.enabled && !is_loopback_host(&http.host) {
        add(
            Severity::High,
            "adapter-exposed",
            format!("HTTP adapter bound to a non-loopback address ({})", http.host),
    ...
    if any_listener && config.access.api_keys.is_empty() {
        ...
        add(
            if exposed { Severity::High } else { Severity::Medium },
            "no-api-keys",
    ...
    for key in &config.access.api_keys {
        if key.key.trim().is_empty() {
            ...
            add(
                Severity::Medium,
                "weak-api-key",
                format!("API key '{}' is short ({} chars)", key.name, key.key.trim().len()),
        ...
        add(
            ...
            "An observer with only `read` scope sees full LLM responses and \
             notification bodies on the `/v1/events` stream."
                .to_string(),
            "Set `adapters.http.sse_redact_previews = true` for shared deployments."
                .to_string(),
        );
    }
        ...
        add(
            ...
            "Stored memory (facts, episodes) is written to disk unencrypted; anyone \
             who can read `~/.brain` can read everything Brain knows."
                .to_string(),
            "Set `encryption.enabled = true` to encrypt the store at rest."
                .to_string(),
        );
    }
    ...
    if a.terminal.enabled {
        add(
            ...
            "AI agents can open interactive PTY sessions and run commands through \
             the bridge."
                .to_string(),
            "Disable `adapters.terminal.enabled` if PTY access is not needed."
                .to_string(),
        );
    }
    ...
    if config.actions.messaging.enabled {
        add(
            ...
            "Disable `actions.messaging.enabled` to prevent outbound delivery."
                .to_string(),
        );
    }
...
fn secret_findings(config: &BrainConfig, add: &mut impl FnMut(Severity, &'static str, String, String, String)) {
    ...
    for r in &config.channel.relays {
        if !r.api_key.trim().is_empty() {
            add(
                ...
                "Avoid committing the config, or rotate the token if it has been."
                    .to_string(),
            );
        }
    }
...
/// allowlist permits.
fn exec_findings(config: &BrainConfig, add: &mut impl FnMut(Severity, &'static str, String, String, String)) {
    ...
    if !shell.is_empty() {
        add(
            Severity::High,
            "exec-shell-allowed",
            format!("Exec allowlist permits a shell / privilege tool: {}", shell.join(", ")),
...
const ARBITRARY_CODE: &[&str] = &[
    "python", "python3", "ruby", "perl", "node", "npm", "npx", "php", "lua", "rscript",
    "osascript", "env", "xargs", "awk", "gawk", "sed", "find",
];
